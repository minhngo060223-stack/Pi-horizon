...
GOLDFLAGS :=	-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.Version=${REPOSITORY_VERSION}' \
				-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.CommitHash=${REPOSITORY_COMMIT_HASH}' \
				-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.BuildTimestamp=${BUILD_TIMESTAMP}' \
				-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.Branch=${REPOSITORY_BRANCH}' \
				-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.RSSorobanEnvVersionPrev=${RS_ENV_VERSION_PREV}' \
				-X 'github.com/Pi-Defi-world/suban-rpc/cmd/stellar-rpc/internal/config.RSSorobanEnvVersionCurr=${RS_ENV_VERSION_CURR}'
...
# the build-suban-rpc build target is an optimized build target used by
# the Suban Data Platform (https://suban.org)
build-suban-rpc: build-libs
	go build -ldflags="${GOLDFLAGS}" ${MACOS_MIN_VER} -o ${PI_RPC_BINARY} -trimpath -v ./cmd/stellar-rpc
...
.PHONY: clean fmt watch test rust-test go-test check rust-check go-check install build build-suban-rpc build-libs lint lint-changes
